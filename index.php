    ...
    switch ($_GET["add_to_cart"])
    {
        case "46":
            if (empty($_COOKIE['panier']['pecanNuts'])){
                setCookie("panier[pecanNuts]", 1, strtotime("+5 minutes"));
                header ("location: index.php");
                break;
            }
            $_COOKIE['panier']["pecanNuts"]++;
            setCookie("panier[pecanNuts]",$_COOKIE["panier"]["pecanNuts"], strtotime("+5 minutes"));
            break;
        case "36":
            if (empty($_COOKIE['panier']['chocolateChips'])){
                setCookie("panier[chocolateChips]", 1, strtotime("+5 minutes"));
                header ("location: index.php");
                break;
            }
            $_COOKIE['panier']["chocolateChips"]++;
            setCookie("panier[chocolateChips]",$_COOKIE["panier"]["chocolateChips"], strtotime("+5 minutes"));
            break;
        case "58":
            if (empty($_COOKIE['panier']['chocolateCookie'])){
                setCookie("panier[chocolateCookie]", 1, strtotime("+5 minutes"));
                header ("location: index.php");
                break;
            }
            $_COOKIE['panier']["chocolateCookie"]++;
            setCookie("panier[chocolateCookie]",$_COOKIE["panier"]["chocolateCookie"], strtotime("+5 minutes"));
            break;
        case "32":
            if (empty($_COOKIE['panier']['mmsCookies'])){
                setCookie("panier[mmsCookies]", 1, strtotime("+5 minutes"));
                header ("location: index.php");
                break;
            }
            $_COOKIE['panier']["mmsCookies"]++;
            setCookie("panier[mmsCookies]",$_COOKIE["panier"]["mmsCookies"], strtotime("+5 minutes"));
            break;
    }
// SI GET VIDE : RIEN
}
  ...
  <div class="row">
      ...
      <figure class="thumbnail text-center">
        ...
        <figcaption class="caption">
          <h3>Cookies au noix de pécan</h3>
            <h4><?= $_COOKIE['panier']['pecanNuts'] ?? '' ?></h4>
          <p>Cuisinés par Penny !</p>
    ...
    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
      <figure class="thumbnail text-center">
        ...
        <figcaption class="caption">
          <h3>Cookies Bonne maman</h3>
            <h4><?= $_COOKIE['panier']['chocolateChips'] ?? '' ?></h4>
          <p>Cuisinés avec amour par Bernadette !</p>
            <form role="form" action="" method="GET">
    ...
    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
      <figure class="thumbnail text-center">
        ...
        <figcaption class="caption">
          <h3>Cookies M&N's&copy;</h3>
            <h4><?= $_COOKIE['panier']['mmsCookies'] ?? '' ?></h4>
          <p>Cuisiné par Penny !</p>
            <form role="form" action="" method="GET">
